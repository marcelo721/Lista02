...
public class JavalarSystem {
	...
	private int maximumBugsAnddevs = 0;
	private String path;
	...
	public void moverPlanetas() {
		...
		if (arquives.getNumberOfInstants().size() >= 7) {
			...
			JOptionPane.showMessageDialog(null, "the file has already been read completely, please read another file.",
	...
	public void createBubgs() {
		...
		if (arquives.getNumberOfBugs().size() > 0) {
			...
			maximumBugsAnddevs = maximumBugsAnddevs + quant;
		}

		if (maximumBugsAnddevs < 256 && arquives.getNumberOfBugs().size() > 0) {
	...
	public void createDevs() {
		...
		if (maximumBugsAnddevs < 256 && arquives.getNumberOfDevs().size() > 0) {
	...
	public void enviarDados() {
		...
		planetaDAO.inserir(planetary.getPlanet());
	}
